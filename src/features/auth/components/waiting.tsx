...
export function Waiting({
  ...
  const { mutate: auth, isPending: isAuthPending } = useAuth(
    callbackURL,
    () => {
      setCountdown(30);
    },
  ...
  const { mutate: emergencyAuth, isPending: isEmergencyPending } =
    useEmergencyAuth(callbackURL, () => {
      setCountdown(30);
    });
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-[#666666] text-lg">
            {isEmergency
              ? t("description_emergency", { email })
              : t("description_standard", { email })}
          </p>
        </div>

        <div className="bg-zinc-50 p-6 border border-zinc-100 space-y-4">
          <p className="text-sm text-zinc-500 font-medium">
            {isEmergency ? t("help_emergency") : t("help_standard")}
          </p>
