...
export default function MagicLinkEmail({ name, magicLink, messages, layoutMessages }: MagicLinkEmailProps) {
	return (
		<Layout preview={messages.preview} messages={layoutMessages}>
			<Heading className="text-3xl font-black tracking-tighter text-foreground mb-6">{messages.heading}</Heading>
			<Section className="mb-8">
				<Text className="text-foreground text-lg font-medium leading-relaxed mb-6">
					{name}, <br />
					{messages.greeting}
				</Text>
				<Text className="text-foreground/80 text-base mb-8">{messages.waitingConfirmation}</Text>
