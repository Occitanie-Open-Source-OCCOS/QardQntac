...
export function TagPopover({
  ...
  const { execute: execAssign, isPending: isAssigning } = useAction(
    assignTags,
    {
      onSuccess: onMutated,
      onError: ({ error }) =>
        toast.error(error.serverError ?? t("error_generic")),
    },
  );
    ...
    <Popover>
      <PopoverTrigger
        render={
          <button
            type="button"
            className="inline-flex items-center justify-center size-4.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-primary"
          />
        }
        title={t("assign_tags")}
      >
            ...
            editingId === tag.id ? (
              <div key={tag.id} className="flex gap-1 items-center">
                ...
                <button
                  ...
                  onClick={() =>
                    editName.trim() &&
                    execUpdate({ id: tag.id, name: editName.trim() })
                  }
              ...
              <div
                key={tag.id}
                className="flex items-center gap-1.5 px-1 py-0.5 rounded hover:bg-muted group"
              >
                <button
                  type="button"
          ...
          <div className="mt-2 pt-2 border-t border-border flex flex-col gap-2">
            ...
            <div className="flex gap-1">
              <Button
                ...
                onClick={() =>
                  execCreate({ name: newName.trim(), color: newColor })
                }
              >
                {t("create")}
              </Button>
